...
public class IntervalTree {
	...
	((OtherIntervalTreeNode) node).setHeight(((OtherIntervalTreeNode) node)
		.getHeight() + 1);
    ...
    // It makes the y the new root of the subtree, with x as the new left child
    // of y
    // and y's left child becomes x's right child
    // burcak: left rotate edilen node x, right child'inin y diyelim, left
    // child'i oluyor
    ...
    // burcak: right rotate edilen node x left child'inin y diyelim, right
    // child'i oluyor
	...
	while (x != tree.getRoot() && x.getColor() == Commons.BLACK) {
		...
		if (w.getLeft().getColor() == Commons.BLACK
			&& w.getRight().getColor() == Commons.BLACK) {
		...
		// case2: w is black and both of w's children are black
		if (w.getRight().getColor() == Commons.BLACK
			&& w.getLeft().getColor() == Commons.BLACK) {
		    w.setColor(Commons.RED); // Case2
		    x = x.getParent(); // Case2
		} else {
	...
	while (!NodeName.SENTINEL.equals(y.getNodeName())
		&& (x == y.getRight())) {
	    x = y;
	    y = y.getParent();
	}
     ...
     */
    public IntervalTreeNode intervalTreeDelete(IntervalTree tree,
	    IntervalTreeNode z) {
	...
	// node z
	tree.setNumberofNonOverlappingBases(tree
		.getNumberofNonOverlappingBases() - z.getNumberofBases());
	...
	if (NodeName.SENTINEL.equals(z.getLeft().getNodeName())
		|| NodeName.SENTINEL.equals(z.getRight().getNodeName())) {
	    y = z;
	} else {
	...
	if ((x instanceof OtherIntervalTreeNode)
		&& (y instanceof OtherIntervalTreeNode)) {
	    ((OtherIntervalTreeNode) x).setHeight(((OtherIntervalTreeNode) (y
	...
	if (y != z) {
	    ...
	    if ((z instanceof DnaseIntervalTreeNode)
		    && (y instanceof DnaseIntervalTreeNode)) {
		((DnaseIntervalTreeNode) z)
		.setCellLineName(((DnaseIntervalTreeNode) y)
			.getCellLineName());
		((DnaseIntervalTreeNode) z)
		.setFileName(((DnaseIntervalTreeNode) y).getFileName());
	    }

	    else if ((z instanceof TforHistoneIntervalTreeNode)
		    && (y instanceof TforHistoneIntervalTreeNode)) {
		((TforHistoneIntervalTreeNode) z)
		.setTfbsorHistoneName(((TforHistoneIntervalTreeNode) y)
			.getTfbsorHistoneName());
		((TforHistoneIntervalTreeNode) z)
		.setCellLineName(((TforHistoneIntervalTreeNode) y)
			.getCellLineName());
		((TforHistoneIntervalTreeNode) z)
		.setFileName(((TforHistoneIntervalTreeNode) y)
			.getFileName());
	    }

	    else if ((z instanceof UcscRefSeqGeneIntervalTreeNode)
		    && (y instanceof UcscRefSeqGeneIntervalTreeNode)) {
		((UcscRefSeqGeneIntervalTreeNode) z)
		.setRefSeqGeneName(((UcscRefSeqGeneIntervalTreeNode) y)
			.getRefSeqGeneName());
		((UcscRefSeqGeneIntervalTreeNode) z)
		.setGeneEntrezId(((UcscRefSeqGeneIntervalTreeNode) y)
			.getGeneEntrezId());
		((UcscRefSeqGeneIntervalTreeNode) z)
		.setGeneHugoSymbol(((UcscRefSeqGeneIntervalTreeNode) y)
			.getGeneHugoSymbol());
		((UcscRefSeqGeneIntervalTreeNode) z)
		.setIntervalName(((UcscRefSeqGeneIntervalTreeNode) y)
			.getIntervalName());
		((UcscRefSeqGeneIntervalTreeNode) z)
		.setStrand(((UcscRefSeqGeneIntervalTreeNode) y)
			.getStrand());
	    }
	...
	// The node y is returned so that calling procedure can recyle it via
	// the free list.
	return y;
	...
	// This part sets whether the new inserted node is the left or right
	// child of parent
	...
	if (z instanceof OtherIntervalTreeNode) {
	    ...
	    ((OtherIntervalTreeNode) z).setHeight(((OtherIntervalTreeNode) (z
		    .getParent())).getHeight() + 1);
	...
	try {
	    ...
	    if (node.getNodeName().isNotSentinel()) {
		bufferedWriter.write(node.getLow() + "\t" + node.getHigh()
			+ "\t" + node.getMax()
    ...
    public void intervalTreeInfixTraversal(IntervalTreeNode node,
	    BufferedWriter bufferedWriter, String type,
	...
	try {
		...
		if (Commons.DNASE.equals(type)) {
		    ...
		    bufferedWriter.write(castedNodeDnase.getChromName()
			    .convertEnumtoString()
			    + "\t"
			    + castedNodeDnase.getLow()
			    + "\t"
			    + castedNodeDnase.getHigh()
			    + "\t"
			    + castedNodeDnase.getCellLineName()
			    + "\t"
			    + castedNodeDnase.getFileName()
			    + System.getProperty("line.separator"));
		} else if (Commons.TF.equals(type)) {
		    castedNodeTforHistone = (TforHistoneIntervalTreeNode) node;
		    bufferedWriter.write(castedNodeTforHistone.getChromName()
			    .convertEnumtoString()
			    + "\t"
			    + castedNodeTforHistone.getLow()
			    + "\t"
			    + castedNodeTforHistone.getHigh()
			    + "\t"
			    + castedNodeTforHistone.getTfbsorHistoneName()
			    + "\t"
			    + castedNodeTforHistone.getCellLineName()
			    + "\t"
			    + castedNodeTforHistone.getFileName()
			    + System.getProperty("line.separator"));
		    ...
		    bufferedWriter.write(castedNodeUcscRefSeqGene
			    .getChromName().convertEnumtoString()
			    + "\t"
			    + castedNodeUcscRefSeqGene.getLow()
			    + "\t"
			    + castedNodeUcscRefSeqGene.getHigh()
			    + "\t"
			    + castedNodeUcscRefSeqGene.getRefSeqGeneName()
			    + "\t"
			    + castedNodeUcscRefSeqGene.getGeneEntrezId()
			    + "\t"
			    + castedNodeUcscRefSeqGene.getIntervalName()
			    .convertEnumtoString()
			    + "\t"
			    + castedNodeUcscRefSeqGene.getIntervalNumber()
			    + "\t"
			    + castedNodeUcscRefSeqGene.getStrand()
			    + "\t"
			    + castedNodeUcscRefSeqGene.getGeneHugoSymbol()
			    + System.getProperty("line.separator"));
		    ...
		    // System.getProperty("line.separator"));
		} else if (Commons.PROCESS_INPUT_DATA_REMOVE_OVERLAPS
			.equals(type)) {
		    bufferedWriter.write(node.getChromName()
			    .convertEnumtoString()
			    + "\t"
			    + node.getLow()
			    + "\t"
    ...
    public static boolean overlaps(int low_x, int high_x, int low_y,
	    int high_y, int numberofOverlappingBases) {
	if ((low_x <= high_y) && (low_y <= high_x)) {
    ...
    // For debug purposes
    public static void findGivenNode(IntervalTreeNode node, int low, int high,
	...
	if (node.getLeft().getNodeName().isNotSentinel()
		&& ((OtherIntervalTreeNode) (node.getLeft())).getMin() <= high
		&& low <= node.getLeft().getMax()) {
	    findGivenNode(node.getLeft(), low, high, foundNodes);
	}

	// partial overlap possibility
	if (node.getRight().getNodeName().isNotSentinel()
		&& ((OtherIntervalTreeNode) (node.getRight())).getMin() <= high
		&& low <= node.getRight().getMax()) {
	...
	IntervalTreeNode leftMostNode = previousNonEmptyOverlappingNodeList
		.get(0);
    ...
    public static IntervalTreeNode findMostGeneralSearchStaringNodeforNewQuery(
	    Interval interval, IntervalTreeNode leftMostNode) {
    ...
    public static IntervalTreeNode findMostGeneralSearchStaringNodeforNewQuery(
	    Interval interval, IntervalTreeNode leftMostNode,
	    IntervalTreeNode rightMostNode) {
	// For OCD GWAS taking right most node did not work
	// Test for POSITIVE CONTROL DATA
    ...
    public static IntervalTreeNode findMostSpecificSearchStaringNodeforNewQuery(
	    Interval interval, IntervalTreeNode node) {

	// if there is possibility of overlap between interval and node's
	// children
	// or overlaps with the node
	if (((node.getLeft().getNodeName().isNotSentinel())
		&& (node.getRight().getNodeName().isNotSentinel())
		&& ((((OtherIntervalTreeNode) (node.getLeft())).getMin() <= interval
		.getHigh()) && (interval.getLow() <= node.getLeft()
		.getMax())) && ((((OtherIntervalTreeNode) (node
			.getRight())).getMin() <= interval.getHigh()) && (interval
				.getLow() <= node.getRight().getMax())))
				|| (node.getLow() <= interval.getHigh() && interval.getLow() <= node
				.getHigh())) {
	    return node;
		...
		.getHigh()) && (interval.getLow() <= node.getLeft()
		.getMax()))) {
	    return findMostSpecificSearchStaringNodeforNewQuery(interval,
		    node.getLeft());
	} else if (node.getRight().getNodeName().isNotSentinel()
		&& ((((OtherIntervalTreeNode) (node.getRight())).getMin() <= interval
		.getHigh()) && (interval.getLow() <= node.getRight()
		.getMax()))) {
	    return findMostSpecificSearchStaringNodeforNewQuery(interval,
		    node.getRight());
	}
    ...
    public static void printRouteFromRoottoThisNode(IntervalTreeNode root,
	    int low, int high, List<IntervalTreeNode> foundNodes) {
    ...
    // Normal
    public void findAllOverlappingIntervals(IntervalTreeNode node,
	    Interval interval) {
	if (node.getNodeName().isNotSentinel()) {
	    if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
		    interval.getHigh())) {
		GlanetRunner.appendLog("overlap" + node.getLow() + "\t"
			+ node.getHigh());
	    }

	    if ((node.getLeft().getNodeName().isNotSentinel())
		    && (interval.getLow() <= node.getLeft().getMax())) {
		findAllOverlappingIntervals(node.getLeft(), interval);
	    }

	    if ((node.getRight().getNodeName().isNotSentinel())
		    && (interval.getLow() <= node.getRight().getMax())
		    && (node.getLow() <= interval.getHigh())) {
    ...
    public void findAllOverlappingIntervals(
	    List<IntervalTreeNode> overlappedNodeList, IntervalTreeNode root,
	    IntervalTreeNode newNode) {

	if (root.getNodeName().isNotSentinel()
		&& newNode.getNodeName().isNotSentinel()) {
	    if (overlaps(root.getLow(), root.getHigh(), newNode.getLow(),
		    newNode.getHigh())) {
		overlappedNodeList.add(root);
	    }

	    if ((root.getLeft().getNodeName().isNotSentinel())
		    && (newNode.getLow() <= root.getLeft().getMax())) {
		findAllOverlappingIntervals(overlappedNodeList, root.getLeft(),
			newNode);
	    }

	    if ((root.getRight().getNodeName().isNotSentinel())
		    && (newNode.getLow() <= root.getRight().getMax())
		    && (root.getLow() <= newNode.getHigh())) {
		findAllOverlappingIntervals(overlappedNodeList,
			root.getRight(), newNode);
    ...
    public IntervalTreeNode findFirstOverlappingIntervals(
	    IntervalTreeNode root, IntervalTreeNode newNode) {
	...
	if (root.getNodeName().isNotSentinel()) {
		...
		if ((root.getLeft().getNodeName().isNotSentinel())
			&& (newNode.getLow() <= root.getLeft().getMax())) {
		    overlappedNode = findFirstOverlappingIntervals(
			    root.getLeft(), newNode);
    ...
    // Search1
    public void findAllOverlappingHistoneIntervals(IntervalTreeNode node,
	    Interval interval, BufferedWriter bufferedWriter,
	...
	if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
	    ...
	    try {
		bufferedWriter.write("histone" + "\t"
			+ castedNode.getChromName().toString() + "\t"
			+ castedNode.getLow() + "\t" + castedNode.getHigh()
			+ "\t" + castedNode.getTfbsorHistoneName().toString()
			+ "\t" + castedNode.getCellLineName().toString() + "\t"
			+ castedNode.getFileName().toString()
			+ System.getProperty("line.separator"));
		bufferedWriter.flush();
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingHistoneIntervals(node.getLeft(), interval,
	...
	if ((node.getRight().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getRight().getMax())
		&& (node.getLow() <= interval.getHigh())) {
	    findAllOverlappingHistoneIntervals(node.getRight(), interval,
		    bufferedWriter, overlappingNodeList);
    ...
    // with IO
    public void findAllOverlappingHistoneIntervalsWithIOWithNumbers(
	    String outputFolder,
	...
	if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
		interval.getHigh(), overlapDefinition)) {
		...
		bufferedWriter = bufferedWriterHashMap
			.get(permutationNumberHistoneNumberCellLineNumber);

		if (bufferedWriter == null) {

		    fileWriter = FileOperations
			    .createFileWriter(
				    outputFolder
				    + Commons.ANNOTATE_PERMUTATIONS_FOR_HISTONE
				    + Commons.PERMUTATION
				    + permutationNumber
				    + System.getProperty("file.separator")
				    + permutationNumberHistoneNumberCellLineNumber
				    + ".txt", true);
		    bufferedWriter = new BufferedWriter(fileWriter);
		    bufferedWriter.write("Searched for" + "\t" + "chromName"
			    + "\t" + "intervalLow" + "\t" + "intervalHigh"
			    + "\t" + "histone" + "\t" + "ChromName" + "\t"
			    + "Low" + "\t" + "High" + "\t" + "HistoneNumber"
			    + "\t" + "CellLineNumber" + "\t" + "FileNumber"
			    + System.getProperty("line.separator"));
		    bufferedWriter.flush();

		    bufferedWriterHashMap.put(
			    permutationNumberHistoneNumberCellLineNumber,
			    bufferedWriter);
		}

		if (!(permutationNumberHistoneNumberCellLineNumber2ZeroorOneMap
			.containsKey(permutationNumberHistoneNumberCellLineNumber))) {
		    permutationNumberHistoneNumberCellLineNumber2ZeroorOneMap
		    .put(permutationNumberHistoneNumberCellLineNumber,
			    1);
		}

		bufferedWriter.write("Searched for" + "\t" + chromName + "\t"
			+ interval.getLow() + "\t" + interval.getHigh() + "\t"
			+ "histone" + "\t" + castedNode.getChromName() + "\t"
			+ castedNode.getLow() + "\t" + castedNode.getHigh()
			+ "\t" + castedNode.getTforHistoneNumber() + "\t"
			+ castedNode.getCellLineNumber() + "\t"
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingHistoneIntervalsWithIOWithNumbers(outputFolder,
		    permutationNumber, node.getLeft(), interval, chromName,
		    bufferedWriterHashMap,
		    permutationNumberHistoneNumberCellLineNumber2ZeroorOneMap,
		    overlapDefinition);
	}
	...
	if ((node.getRight().getNodeName().isNotSentinel())
		...
		&& (node.getLow() <= interval.getHigh())) {
	    findAllOverlappingHistoneIntervalsWithIOWithNumbers(outputFolder,
		    permutationNumber, node.getRight(), interval, chromName,
		    bufferedWriterHashMap,
		    permutationNumberHistoneNumberCellLineNumber2ZeroorOneMap,
		    overlapDefinition);
    ...
    public void findAllOverlappingHistoneIntervals(
	    ...
	    Interval interval,
	    ChromosomeName chromName,
	    Map<String, BufferedWriter> bufferedWriterHashMap,
	...
	if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
	    ...
	    try {
		permutationNumberHistoneNameaCellLineName = Commons.PERMUTATION
			+ permutationNumber + "_"
			+ castedNode.getTfbsorHistoneName() + "_"
			+ castedNode.getCellLineName();

		bufferedWriter = bufferedWriterHashMap
			.get(permutationNumberHistoneNameaCellLineName);
		...
		if (bufferedWriter == null) {
		    ...
		    fileWriter = FileOperations.createFileWriter(
			    outputFolder
			    + Commons.ANNOTATE_PERMUTATIONS_FOR_HISTONE
			    + Commons.PERMUTATION + permutationNumber
			    + System.getProperty("file.separator")
			    + permutationNumberHistoneNameaCellLineName
			    + ".txt", true);
		    bufferedWriter = new BufferedWriter(fileWriter);
		    bufferedWriterHashMap.put(
			    permutationNumberHistoneNameaCellLineName,
		...
		if (permutationNumberHistoneNameCellLineName2ZeroorOneMap
			.get(permutationNumberHistoneNameaCellLineName) == null) {
		...
		bufferedWriter.write("Searched for" + "\t" + chromName + "\t"
			...
			+ "histone" + "\t" + castedNode.getChromName() + "\t"
			+ castedNode.getLow() + "\t" + castedNode.getHigh()
			+ "\t" + castedNode.getTfbsorHistoneName() + "\t"
			+ String.valueOf(castedNode.getCellLineName()) + "\t"
			+ castedNode.getFileName()
			+ System.getProperty("line.separator"));
		bufferedWriter.flush();
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingHistoneIntervals(outputFolder, permutationNumber,
		    node.getLeft(), interval, chromName, bufferedWriterHashMap,
		    permutationNumberHistoneNameCellLineName2ZeroorOneMap,
		    overlapDefinition);
	}
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingHistoneIntervals(permutationNumber,
	...
	if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
	    ...
	    permutationNumberHistoneNameaCellLineName = Commons.PERMUTATION
		    + (((repeatNumber - 1) * NUMBER_OF_PERMUTATIONS) + permutationNumber)
		    + "_" + castedNode.getTfbsorHistoneName() + "_"
		    + castedNode.getCellLineName();
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingHistoneIntervals(repeatNumber, permutationNumber,
		    NUMBER_OF_PERMUTATIONS, node.getLeft(), interval,
		    chromName,
		    permutationNumberHistoneNameCellLineName2ZeroorOneMap,
		    overlappingNodeList);
	}
	...
	if ((node.getRight().getNodeName().isNotSentinel())
		...
		&& (node.getLow() <= interval.getHigh())) {
	    findAllOverlappingHistoneIntervals(repeatNumber, permutationNumber,
		    NUMBER_OF_PERMUTATIONS, node.getRight(), interval,
		    chromName,
		    permutationNumberHistoneNameCellLineName2ZeroorOneMap,
		    overlappingNodeList);
    ...
    public void findAllOverlappingHistoneIntervalsWithNumbers(
	    String outputFolder, IntervalTreeNode node, Interval interval,
	    ChromosomeName chromName,
	    TIntObjectMap<BufferedWriter> bufferedWriterHashMap,
	    TIntShortMap histoneNumberCellLineNumber2ZeroorOneMap,
	    int overlapDefinition,
	...
	if (overlaps(castedNode.getLow(), castedNode.getHigh(),
		interval.getLow(), interval.getHigh(), overlapDefinition)) {
	    try {

		histoneName = histoneNumber2HistoneNameMap.get(castedNode
			.getTforHistoneNumber());
		cellLineName = cellLineNumber2CellLineNameMap.get(castedNode
			.getCellLineNumber());
		fileName = fileNumber2FileNameMap.get(castedNode
			.getFileNumber());

		bufferedWriter = bufferedWriterHashMap
			.get(histoneNumberCellLineNumber);

		if (bufferedWriter == null) {
		    fileWriter = FileOperations.createFileWriter(outputFolder
			    + Commons.HISTONE_ANNOTATION_DIRECTORY
			    + histoneName + "_" + cellLineName + ".txt", true);
		    bufferedWriter = new BufferedWriter(fileWriter);
		    bufferedWriterHashMap.put(histoneNumberCellLineNumber,
			    bufferedWriter);
		    bufferedWriter.write("#Searched for chr" + "\t"
			    + "interval low" + "\t" + "interval high" + "\t"
			    + "histone node chrom name" + "\t" + "node Low"
			    + "\t" + "node high" + "\t" + "node HistoneName"
			    + "\t" + "node CellLineName" + "\t"
			    + "node FileName"
			    + System.getProperty("line.separator"));
		...
		if (!histoneNumberCellLineNumber2ZeroorOneMap
			.containsKey(histoneNumberCellLineNumber)) {
		    histoneNumberCellLineNumber2ZeroorOneMap.put(
			    histoneNumberCellLineNumber, (short) 1);
		}

		bufferedWriter.write(chromName.convertEnumtoString()
			+ "\t"
			+ interval.getLow()
			+ "\t"
			+ interval.getHigh()
			+ "\t"
			+ ChromosomeName.convertEnumtoString(castedNode
				.getChromName()) + "\t" + castedNode.getLow()
				+ "\t" + castedNode.getHigh() + "\t" + histoneName
				+ "\t" + cellLineName + "\t" + fileName
				+ System.getProperty("line.separator"));
		bufferedWriter.flush();
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingHistoneIntervalsWithNumbers(outputFolder,
		    node.getLeft(), interval, chromName, bufferedWriterHashMap,
		    histoneNumberCellLineNumber2ZeroorOneMap,
		    overlapDefinition, histoneNumber2HistoneNameMap,
    ...
    // value 1 or 0
    // These 1 or 0's will be accumulated in histoneNameandCellLineName2KMap
    public void findAllOverlappingHistoneIntervals(String outputFolder,
	    IntervalTreeNode node, Interval interval, ChromosomeName chromName,
	    Map<String, BufferedWriter> bufferedWriterHashMap,
	    List<String> histoneNameList,
	    Map<String, Integer> histoneNameandCellLineName2ZeroorOneMap,
	    int overlapDefinition) {
	FileWriter fileWriter = null;
	BufferedWriter bufferedWriter = null;
	...
	String histoneNameandCellLine = castedNode.getTfbsorHistoneName() + "_"
		+ castedNode.getCellLineName();

	if (overlaps(castedNode.getLow(), castedNode.getHigh(),
		interval.getLow(), interval.getHigh(), overlapDefinition)
		&& histoneNameList.contains(castedNode.getTfbsorHistoneName())) {
	    try {

		bufferedWriter = bufferedWriterHashMap
			.get(histoneNameandCellLine);

		if (bufferedWriter == null) {
		    fileWriter = FileOperations.createFileWriter(outputFolder
			    + Commons.HISTONE_ANNOTATION_DIRECTORY + "_"
			    + histoneNameandCellLine + ".txt");
		    bufferedWriter = new BufferedWriter(fileWriter);
		    bufferedWriterHashMap.put(histoneNameandCellLine,
			    bufferedWriter);
		    bufferedWriter.write("Searched for chr" + "\t"
			    + "interval low" + "\t" + "interval high" + "\t"
			    + "histone node chrom name" + "\t" + "node Low"
			    + "\t" + "node high" + "\t" + "node Histone Name"
			    + "\t" + "node CellLineName" + "\t"
			    + "node FileName"
			    + System.getProperty("line.separator"));
		...
		if (histoneNameandCellLineName2ZeroorOneMap
			.get(histoneNameandCellLine) == null) {
		    histoneNameandCellLineName2ZeroorOneMap.put(
			    histoneNameandCellLine, 1);
		}
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingHistoneIntervals(outputFolder, node.getLeft(),
		    interval, chromName, bufferedWriterHashMap,
		    histoneNameList, histoneNameandCellLineName2ZeroorOneMap,
		    overlapDefinition);
	}
	...
	if ((node.getRight().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getRight().getMax())
		&& (node.getLow() <= interval.getHigh())) {
	    findAllOverlappingHistoneIntervals(outputFolder, node.getRight(),
		    interval, chromName, bufferedWriterHashMap,
		    histoneNameList, histoneNameandCellLineName2ZeroorOneMap,
		    overlapDefinition);
    ...
    public void findAllOverlappingTfbsIntervalsWithIOWithNumbers(
	    String outputFolder,
	    int permutationNumber,
	    IntervalTreeNode node,
	    Interval interval,
	...
	if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
		...
		.add(new PermutationNumberTfNumberCellLineNumberOverlap(
			permutationNumberTfNumberCellLineNumber,
			castedNode.getLow(), castedNode.getHigh()));

		bufferedWriter = bufferedWriterHashMap
			.get(permutationNumberTfNumberCellLineNumber);

		if (bufferedWriter == null) {

		    fileWriter = FileOperations.createFileWriter(
			    outputFolder
			    + Commons.ANNOTATE_PERMUTATIONS_FOR_TFBS
			    + Commons.PERMUTATION + permutationNumber
			    + System.getProperty("file.separator")
			    + permutationNumberTfNumberCellLineNumber
			    + ".txt", true);
		...
		if (!(permutationNumberTfNumberCellLineNumber2ZeroorOneMap
			.containsKey(permutationNumberTfNumberCellLineNumber))) {
		    permutationNumberTfNumberCellLineNumber2ZeroorOneMap.put(
			    permutationNumberTfNumberCellLineNumber, 1);
		}

		bufferedWriter.write("Searched for" + "\t" + chromName + "\t"
			+ interval.getLow() + "\t" + interval.getHigh() + "\t"
			+ "tfbs" + "\t" + castedNode.getChromName() + "\t"
			+ castedNode.getLow() + "\t" + castedNode.getHigh()
			+ "\t" + castedNode.getTforHistoneNumber() + "\t"
			+ castedNode.getCellLineNumber() + "\t"
			+ castedNode.getFileNumber()
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingTfbsIntervalsWithIOWithNumbers(outputFolder,
		    permutationNumber, node.getLeft(), interval, chromName,
		    bufferedWriterHashMap,
    ...
    // with IO
    // with OverlapNodeList
    public void findAllOverlappingTfbsIntervals(
	    String outputFolder,
	    int permutationNumber,
	    IntervalTreeNode node,
	    Interval interval,
	...
	if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
		...
		permutationNumberTfNameCellLineName = Commons.PERMUTATION
			+ permutationNumber + "_"
			+ castedNode.getTfbsorHistoneName() + "_"
			+ castedNode.getCellLineName();

		permutationNumberTfNameCellLineNameOverlapList
		.add(new PermutationNumberTfNameCellLineNameOverlap(
			permutationNumberTfNameCellLineName, castedNode
			.getLow(), castedNode.getHigh()));

		bufferedWriter = bufferedWriterHashMap
			.get(permutationNumberTfNameCellLineName);
		...
		if (bufferedWriter == null) {
		    ...
		    fileWriter = FileOperations.createFileWriter(
			    ...
			    + Commons.PERMUTATION + permutationNumber
			    + System.getProperty("file.separator")
			    + permutationNumberTfNameCellLineName
			    + ".txt", true);
		    bufferedWriter = new BufferedWriter(fileWriter);
		    bufferedWriterHashMap
		    .put(permutationNumberTfNameCellLineName,
			    bufferedWriter);
		}

		if (permutationNumberTfbsNameCellLineName2ZeroorOneMap
			.get(permutationNumberTfNameCellLineName) == null) {
		    permutationNumberTfbsNameCellLineName2ZeroorOneMap.put(
			    permutationNumberTfNameCellLineName, 1);
		}
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingTfbsIntervals(outputFolder, permutationNumber,
		    node.getLeft(), interval, chromName, bufferedWriterHashMap,
    ...
    // Empirical P Value Calculation
    public void findAllOverlappingTfbsIntervalsWithIOWithNumbers(
	    String outputFolder, int permutationNumber, IntervalTreeNode node,
	...
	if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
		...
		permutationNumberTfbsNameCellLineName = Commons.PERMUTATION
			+ permutationNumber + "_"
			+ castedNode.getTfbsorHistoneName() + "_"
			+ castedNode.getCellLineName();

		bufferedWriter = bufferedWriterHashMap
			.get(permutationNumberTfbsNameCellLineName);
		...
		if (bufferedWriter == null) {
		    ...
		    fileWriter = FileOperations.createFileWriter(
			    ...
			    + Commons.PERMUTATION + permutationNumber
			    + System.getProperty("file.separator")
			    + permutationNumberTfbsNameCellLineName
			    + ".txt", true);
		    bufferedWriter = new BufferedWriter(fileWriter);
		    bufferedWriterHashMap.put(
			    permutationNumberTfbsNameCellLineName,
		...
		if (permutationNumberTfbsNameCellLineName2ZeroorOneMap
			.get(permutationNumberTfbsNameCellLineName) == null) {
		    permutationNumberTfbsNameCellLineName2ZeroorOneMap.put(
			    permutationNumberTfbsNameCellLineName, 1);
		}
    ...
    public void findAllOverlappingTfbsIntervalsWithoutIOWithNumbers(
	    int permutationNumber, IntervalTreeNode node, Interval interval,
	    ChromosomeName chromName,
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingTfbsIntervals(permutationNumber, node.getLeft(),
		    interval, chromName,
		    permutationNumberTfbsNameCellLineName2ZeroorOneMap,
		    overlapDefinition);
	}
	...
	if ((node.getRight().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getRight().getMax())
		&& (node.getLow() <= interval.getHigh())) {
	    findAllOverlappingTfbsIntervals(permutationNumber, node.getRight(),
		    interval, chromName,
		    permutationNumberTfbsNameCellLineName2ZeroorOneMap,
		    overlapDefinition);
	...
	if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
	    ...
	    permutationNumberTfNumberCellLineNumberOverlapList
	    .add(new PermutationNumberTfNumberCellLineNumberOverlap(
		    permutationNumberTfNumberCellLineNumber, castedNode
		    .getLow(), castedNode.getHigh()));
    ...
    public void findAllOverlappingTfbsIntervals(IntervalTreeNode node,
	    Interval interval, ChromosomeName chromName,
	    Map<String, BufferedWriter> bufferedWriterHashMap,
	    List<String> tfbsNameList,
	...
	String tfbsNameandCellLineName = castedNode.getTfbsorHistoneName()
		+ "_" + castedNode.getCellLineName();

	if (overlaps(castedNode.getLow(), castedNode.getHigh(),
		interval.getLow(), interval.getHigh())
		&& tfbsNameList.contains(castedNode.getTfbsorHistoneName())) {
	    try {

		bufferedWriter = bufferedWriterHashMap
			.get(tfbsNameandCellLineName);

		if (bufferedWriter == null) {
		    fileWriter = FileOperations
			    .createFileWriter(Commons.TF_ANNOTATION_DIRECTORY
				    + tfbsNameandCellLineName + ".txt");
		    bufferedWriter = new BufferedWriter(fileWriter);
		    bufferedWriterHashMap.put(tfbsNameandCellLineName,
			    bufferedWriter);
		}

		if (tfbsNameandCellLineName2ZeroorOneMap
			.get(tfbsNameandCellLineName) == null) {
		    tfbsNameandCellLineName2ZeroorOneMap.put(
			    tfbsNameandCellLineName, 1);
		}

		bufferedWriter.write("Searched for" + "\t"
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingTfbsIntervals(node.getLeft(), interval,
		    chromName, bufferedWriterHashMap, tfbsNameList,
	...
	if ((node.getRight().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getRight().getMax())
		&& (node.getLow() <= interval.getHigh())) {
	    findAllOverlappingTfbsIntervals(node.getRight(), interval,
		    chromName, bufferedWriterHashMap, tfbsNameList,
		    tfbsNameandCellLineName2ZeroorOneMap);
	...
	if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
	    ...
	    if (!(permutationNumberElementTypeNumberElementNumber2ZeroorOneMap
		    .containsKey(permutationNumberElementTypeNumberElementNumber))) {
		permutationNumberElementTypeNumberElementNumber2ZeroorOneMap
		.put(permutationNumberElementTypeNumberElementNumber, 1);
	    }
	}
	...
	if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
		...
		bufferedWriter = permutationNumberElementTypeNumberElementNumber2BufferedWriterHashMap
			.get(permutationNumberElementTypeNumberElementNumber);
		...
		if (bufferedWriter == null) {
		    ...
		    fileWriter = FileOperations
			    .createFileWriter(
				    outputFolder
				    + Commons.ANNOTATE_PERMUTATIONS_FOR_USERDEFINEDLIBRARY
				    + Commons.PERMUTATION
				    + permutationNumber
				    + System.getProperty("file.separator")
				    + castedNode.getElementTypeNumber()
				    + System.getProperty("file.separator")
				    + permutationNumberElementTypeNumberElementNumber
				    + ".txt", true);
		    bufferedWriter = new BufferedWriter(fileWriter);
		    bufferedWriter.write("Searched for" + "\t" + "chromName"
			    + "\t" + "intervalLow" + "\t" + "intervalHigh"
			    + "\t" + "UserDefinedLibrary" + "\t" + "ChromName"
			    + "\t" + "Low" + "\t" + "High" + "\t"
			    + "ElementTypeNumber" + "\t" + "ElementNumber"
			    + "\t" + "FileNumber"
			    + System.getProperty("line.separator"));
		    bufferedWriter.flush();

		    permutationNumberElementTypeNumberElementNumber2BufferedWriterHashMap
		    .put(permutationNumberElementTypeNumberElementNumber,
			    bufferedWriter);
		}
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingUserDefinedLibraryIntervalsWithIOWithNumbers(
		    outputFolder,
		    permutationNumber,
		    node.getLeft(),
		    interval,
    ...
    public void findAllOverlappingUserDefinedLibraryIntervalsWithNumbers(
	    ...
	    TIntShortMap elementNumber2ZeroorOneMap, int overlapDefinition,
	    String elementType,
	...
	if (overlaps(castedNode.getLow(), castedNode.getHigh(),
		...
		if (bufferedWriter == null) {
		    fileWriter = FileOperations
			    .createFileWriter(
				    outputFolder
				    + Commons.USERDEFINEDLIBRARY_ANNOTATION_DIRECTORY
				    + elementType
				    + System.getProperty("file.separator")
				    ...
				    + ".txt", true);
		    bufferedWriter = new BufferedWriter(fileWriter);
		    userDefinedLibraryBufferedWriterHashMap.put(elementNumber,
			    bufferedWriter);
		    bufferedWriter.write("#Searched for chr" + "\t"
			    + "interval Low" + "\t" + "interval High" + "\t"
			    + "UserDefinedLibraryNode ChromName" + "\t"
			    + "node Low" + "\t" + "node High" + "\t"
		...
		bufferedWriter
		.write(chromName.convertEnumtoString()
			+ "\t"
			+ interval.getLow()
			+ "\t"
			+ interval.getHigh()
			...
			+ ChromosomeName.convertEnumtoString(castedNode
				...
				.get(elementNumber)
				+ "\t"
				+ fileNumber2FileNameMap.get(castedNode
					.getFileNumber())
					+ System.getProperty("line.separator"));
		bufferedWriter.flush();
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
	    ...
	    findAllOverlappingUserDefinedLibraryIntervalsWithNumbers(
		    outputFolder, node.getLeft(), interval, chromName,
		    userDefinedLibraryBufferedWriterHashMap,
		    elementNumber2ZeroorOneMap, overlapDefinition, elementType,
		    elementNumber2ElementNameMap, fileNumber2FileNameMap);
	}
	...
	if ((node.getRight().getNodeName().isNotSentinel())
		...
		&& (node.getLow() <= interval.getHigh())) {
	    findAllOverlappingUserDefinedLibraryIntervalsWithNumbers(
		    outputFolder, node.getRight(), interval, chromName,
		    userDefinedLibraryBufferedWriterHashMap,
		    elementNumber2ZeroorOneMap, overlapDefinition, elementType,
		    elementNumber2ElementNameMap, fileNumber2FileNameMap);
	}
    ...
    public void findAllOverlappingTfbsIntervalsWithNumbers(String outputFolder,
	    IntervalTreeNode node, Interval interval, ChromosomeName chromName,
	    TIntObjectMap<BufferedWriter> bufferedWriterHashMap,
	    TIntShortMap tfNumberCellLineNumber2ZeroorOneMap,
	...
	int elementNumberCellLineNumber = IntervalTree
		.generateElementNumberCellLineNumberKeggPathwayNumber(
			castedNode.getTforHistoneNumber(),
			castedNode.getCellLineNumber(),
			(short) 0,
			GeneratedMixedNumberDescriptionOrderLength.INT_4DIGITS_ELEMENTNUMBER_3DIGITS_CELLLINENUMBER_3DIGITS_KEGGPATHWAYNUMBER);
	...
	if (overlaps(castedNode.getLow(), castedNode.getHigh(),
		...
		bufferedWriter = bufferedWriterHashMap
			.get(elementNumberCellLineNumber);
		...
		if (bufferedWriter == null) {
		    fileWriter = FileOperations
			    .createFileWriter(
				    ...
				    + tfNumber2TfNameMap.get(castedNode
					    .getTforHistoneNumber())
					    + "_"
					    + cellLineNumber2CellLineNameMap.get(castedNode
						    .getCellLineNumber())
						    + ".txt", true);
		    bufferedWriter = new BufferedWriter(fileWriter);
		    bufferedWriterHashMap.put(elementNumberCellLineNumber,
			    bufferedWriter);
		    bufferedWriter.write("#Searched for chr" + "\t"
			    + "interval Low" + "\t" + "interval High" + "\t"
			    + "tfbs node Chrom Name" + "\t" + "node Low" + "\t"
			    + "node High" + "\t" + "node Tfbs Name" + "\t"
			    + "node CellLineName" + "\t" + "node FileName"
			    + System.getProperty("line.separator"));
		    bufferedWriter.flush();
		}

		if (!tfNumberCellLineNumber2ZeroorOneMap
			.containsKey(elementNumberCellLineNumber)) {
		    tfNumberCellLineNumber2ZeroorOneMap.put(
			    elementNumberCellLineNumber, (short) 1);
		}
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingTfbsIntervalsWithNumbers(outputFolder,
		    node.getLeft(), interval, chromName, bufferedWriterHashMap,
		    tfNumberCellLineNumber2ZeroorOneMap, overlapDefinition,
		    tfNumber2TfNameMap, cellLineNumber2CellLineNameMap,
	...
	if ((node.getRight().getNodeName().isNotSentinel())
	    ...
	    findAllOverlappingTfbsIntervalsWithNumbers(outputFolder,
		    node.getRight(), interval, chromName,
		    bufferedWriterHashMap, tfNumberCellLineNumber2ZeroorOneMap,
		    overlapDefinition, tfNumber2TfNameMap,
		    cellLineNumber2CellLineNameMap, fileNumber2FileNameMap);
	}
    }
	...
	if (overlaps(castedNode.getLow(), castedNode.getHigh(),
		...
		tfandCellLineOverlapList.add(new TfCellLineOverlapWithNumbers(
			elementNumberCellLineNumber, castedNode.getLow(),
			castedNode.getHigh()));
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
	    ...
	    findAllOverlappingTfbsIntervalsWithNumbers(outputFolder,
		    ...
		    tfandCellLineOverlapList, overlapDefinition,
		    tfNumber2TfNameMap, cellLineNumber2CellLineNameMap,
		    fileNumber2FileNameMap);
	}
    ...
    // For each search input line, each tfbs will have value 1 or 0
    // These 1 or 0's will be accumulated in tfbs2KMap
    public void findAllOverlappingTfbsIntervals(String outputFolder,
	    IntervalTreeNode node, Interval interval, ChromosomeName chromName,
	    Map<String, BufferedWriter> bufferedWriterHashMap,
	    List<String> tfbsNameList,
	    Map<String, Integer> tfbsNameandCellLineName2ZeroorOneMap,
	    List<TfNameandCellLineNameOverlap> tfandCellLineOverlapList,
	    int overlapDefinition) {
	FileWriter fileWriter = null;
	BufferedWriter bufferedWriter = null;
	...
	if (overlaps(castedNode.getLow(), castedNode.getHigh(),
		...
		if (bufferedWriter == null) {
		    fileWriter = FileOperations.createFileWriter(outputFolder
			    + Commons.TF_ANNOTATION_DIRECTORY
			    + tfbsNameandCellLineName + ".txt", true);
		    bufferedWriter = new BufferedWriter(fileWriter);
		    bufferedWriterHashMap.put(tfbsNameandCellLineName,
			    bufferedWriter);
		    bufferedWriter.write("Searched for chr" + "\t"
		...
		tfandCellLineOverlapList.add(new TfNameandCellLineNameOverlap(
			tfbsNameandCellLineName, castedNode.getLow(),
			castedNode.getHigh()));
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingTfbsIntervals(outputFolder, node.getLeft(),
		    interval, chromName, bufferedWriterHashMap, tfbsNameList,
		    tfbsNameandCellLineName2ZeroorOneMap,
		    tfandCellLineOverlapList, overlapDefinition);
	}
	...
	if ((node.getRight().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getRight().getMax())
		&& (node.getLow() <= interval.getHigh())) {
	    findAllOverlappingTfbsIntervals(outputFolder, node.getRight(),
		    interval, chromName, bufferedWriterHashMap, tfbsNameList,
		    tfbsNameandCellLineName2ZeroorOneMap,
		    tfandCellLineOverlapList, overlapDefinition);
	}
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingTfbsIntervals(node.getLeft(), interval,
		    bufferedWriter, overlappedNodeList);
	}
    ...
    // Search1
    public void findAllOverlappingDnaseIntervals(IntervalTreeNode node,
	    Interval interval, BufferedWriter bufferedWriter,
	    List<IntervalTreeNode> overlappedNodeList) {
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingDnaseIntervals(node.getLeft(), interval,
		    bufferedWriter, overlappedNodeList);
	}
	...
	if ((node.getRight().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getRight().getMax())
		&& (node.getLow() <= interval.getHigh())) {
	    findAllOverlappingDnaseIntervals(node.getRight(), interval,
		    bufferedWriter, overlappedNodeList);
	}
    ...
    public void findAllOverlappingDnaseIntervalsWithIOWithNumbers(
	    ...
	    Interval interval, ChromosomeName chromName,
	    TIntObjectMap<BufferedWriter> bufferedWriterHashMap,
	    TIntIntMap permutationNumberDnaseCellLineNumber2ZeroorOneMap,
	    int overlapDefinition) {
	FileWriter fileWriter = null;
	BufferedWriter bufferedWriter = null;
	...
	if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
		...
		bufferedWriter = bufferedWriterHashMap
			.get(permutationNumberDnaseCellLineNumber);

		if (bufferedWriter == null) {

		    fileWriter = FileOperations.createFileWriter(
			    outputFolder
			    + Commons.ANNOTATE_PERMUTATIONS_FOR_DNASE
			    + Commons.PERMUTATION + permutationNumber
			    + System.getProperty("file.separator")
			    + permutationNumberDnaseCellLineNumber
			    + ".txt", true);
		    bufferedWriter = new BufferedWriter(fileWriter);
		    bufferedWriter.write("Searched for" + "\t" + "chromName"
			    + "\t" + "intervalLow" + "\t" + "intervalHigh"
			    + "\t" + "dnase" + "\t" + "dnaseChromName" + "\t"
			    + "dnaseLow" + "\t" + "dnaseHigh" + "\t"
			    + "dnaseCellLineNumber" + "\t" + "dnaseFileNumber"
			    + System.getProperty("line.separator"));
		    bufferedWriter.flush();
		...
		if (!(permutationNumberDnaseCellLineNumber2ZeroorOneMap
			.containsKey(permutationNumberDnaseCellLineNumber))) {
		    permutationNumberDnaseCellLineNumber2ZeroorOneMap.put(
			    permutationNumberDnaseCellLineNumber, 1);
		}

		bufferedWriter.write("Searched for" + "\t" + chromName + "\t"
			+ interval.getLow() + "\t" + interval.getHigh() + "\t"
			+ "dnase" + "\t" + castedNode.getChromName() + "\t"
			+ castedNode.getLow() + "\t" + castedNode.getHigh()
			+ "\t" + castedNode.getCellLineNumber() + "\t"
			+ castedNode.getFileNumber()
			+ System.getProperty("line.separator"));
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingDnaseIntervalsWithIOWithNumbers(outputFolder,
		    permutationNumber, node.getLeft(), interval, chromName,
		    bufferedWriterHashMap,
		    permutationNumberDnaseCellLineNumber2ZeroorOneMap,
		    overlapDefinition);
	}
    ...
    public static int removeCellLineNumber(
	...
	elmentNumberCellLineNumber = elementNumberCellLineNumberKeggPathwayNumber
		- keggPathwayNumber;
    ...
    // @todo For Annotation with Numbers starts
    public static int removeElementNumberCellLineNumber(
	    int elementNumberCellLineNumberKeggPathwayNumber) {
	// 1., 2. and 3. digits show keggPathwayNumber
	// 4., 5. and 6. digits show cellLineNumber
	// 7., 8., 9. and 10. digits show elementNumber
    ...
    // @todo For Annotation with Numbers starts
    public static int removeCellLineNumberKeggPathwayNumber(
	    int elementNumberCellLineNumberKeggPathwayNumber) {
	// 1., 2. and 3. digits show keggPathwayNumber
	// 4., 5. and 6. digits show cellLineNumber
	// 7., 8., 9. and 10. digits show elementNumber
	...
	elementNumber = elementNumberCellLineNumberKeggPathwayNumber
		- cellLineNumberKeggPathwayNumber;
    ...
    // Annotation
    // AnnotateGivenIntervals with Numbers
    public static short getElementNumber(
	    int elementNumberCellLineNumberKeggPathwayNumber) {
	// INT_4DIGITS_ELEMENTNUMBER_3DIGITS_CELLLINENUMBER_3DIGITS_KEGGPATHWAYNUMBER
	short elementNumber;
	int cellLineNumberKeggPathwayNumber;
	...
	case LONG_7DIGITS_PERMUTATIONNUMBER_4DIGITS_ELEMENTNUMBER_4DIGITS_CELLLINENUMBER_4DIGITS_KEGGPATHWAYNUMBER:
	    ...
	    int cellLineNumber = getCellLineNumber(
		    permutationNumberTforHistoneNumberCellLineNumberKeggPathwayNumber,
		    generatedMixedNumberDescriptionOrderLength);
	    ...
	    if (cellLineNumber > 0) {
		elementNumberCellLineNumberOrKeggPathwayNumber = elementNumber
			* 10000 + cellLineNumber;
	    } else if (keggPathwayNumber > 0) {
		elementNumberCellLineNumberOrKeggPathwayNumber = elementNumber
			* 10000 + keggPathwayNumber;
	    }
	...
	// PERMUTATIONNUMBER DNASECELLLINENUMBER
	case INT_6DIGITS_PERMUTATIONNUMBER_4DIGITS_CELLLINENUMBER: {
	    permutationNumberCellLineNumberorGeneSetNumber = permutationNumber
		    * 10000 + cellLineNumberorGeneSetNumber;
	    break;
	}
	...
	case LONG_7DIGIT_PERMUTATIONNUMBER_4DIGIT_ELEMENTTYPENUMBER_6DIGIT_ELEMENTNUMBER:
	    ...
	    mixedNumber = _permutationNumber + _elementTypeNumber
		    + _elementNumber;
	    break;
	default:
	    break;
	...
	case LONG_7DIGITS_PERMUTATIONNUMBER_4DIGITS_ELEMENTNUMBER_4DIGITS_CELLLINENUMBER_4DIGITS_KEGGPATHWAYNUMBER: {
	    ...
	    mixedNumber = _permutationNumber + _elementNumber + _cellLineNumber
		    + _keggPathwayNumber;
	...
	case INT_4DIGITS_ELEMENTNUMBER_3DIGITS_CELLLINENUMBER_3DIGITS_KEGGPATHWAYNUMBER: {
	    elementNumberCellLineNumberKeggPathwayNumber = elementNumber
		    * 1000000 + cellLineNumber * 1000 + keggPathwayNumber;
	    break;
	}
	default: {
	    break;
	...
	case LONG_7DIGITS_PERMUTATIONNUMBER_4DIGITS_ELEMENTNUMBER_4DIGITS_CELLLINENUMBER_4DIGITS_KEGGPATHWAYNUMBER: {
	    ...
	    cellLineNumber = cellLineNumberKeggPathwayNumber
		    - keggPathwayNumber;

	    cellLineNumberRemoved = permutationNumberElementNumberCellLineNumberKeggPathwayNumber
		    - cellLineNumber;
	    break;
	}
	default: {
	...
	switch (generatedMixedNumberDescriptionOrderLength) {
	case LONG_7DIGITS_PERMUTATIONNUMBER_4DIGITS_ELEMENTNUMBER_4DIGITS_CELLLINENUMBER_4DIGITS_KEGGPATHWAYNUMBER: {
	    permutationNumberElementNumberCellLineNumberKeggPathwayNumber = permutationNumberElementNumberCellLineNumber
		    + keggPathwayNumber;
	    break;
	}
	default: {
	...
	cellLineRemoved = removeCellLineNumber(mixedNumber,
		generatedMixedNumberDescriptionOrderLength);
	cellLineRemovedKeggPathwayAdded = addKeggPathwayNumber(cellLineRemoved,
		keggPathwayNumber, generatedMixedNumberDescriptionOrderLength);
	...
	if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
	    ...
	    if (!(permutationNumberDnaseCellLineName2ZeroorOneMap
		    .containsKey(permutationNumberDnaseCellLineNumber))) {
		permutationNumberDnaseCellLineName2ZeroorOneMap.put(
			permutationNumberDnaseCellLineNumber, 1);
	    }
	}// End of IF OVERLAPS
    ...
    public void findAllOverlappingDnaseIntervalsWithNumbers(
	    ...
	    Interval interval,
	    ChromosomeName chromName,
	    TShortObjectMap<BufferedWriter> dnaseCellLineNumber2bufferedWriterHashMap,
	    TShortShortMap dnaseCellLineNumber2OneorZeroMap,
	    int overlapDefinition,
	    TShortObjectMap<String> cellLineNumber2CellLineNameMap,
	    TShortObjectMap<String> fileNumber2FileNameMap) {
	FileWriter fileWriter = null;
	...
	if (overlaps(castedNode.getLow(), castedNode.getHigh(),
		...
		bufferedWriter = (BufferedWriter) dnaseCellLineNumber2bufferedWriterHashMap
			.get(castedNode.getCellLineNumber());

		if (bufferedWriter == null) {
		    fileWriter = FileOperations.createFileWriter(outputFolder
			    + Commons.DNASE_ANNOTATION_DIRECTORY + cellLineName
			    + ".txt", true);
		    bufferedWriter = new BufferedWriter(fileWriter);
		    dnaseCellLineNumber2bufferedWriterHashMap.put(
			    castedNode.getCellLineNumber(), bufferedWriter);
		    bufferedWriter.write("#Searched for chr" + "\t"
			    + "given interval low" + "\t"
			    + "given interval high" + "\t"
			    + "dnase overlap chrom name" + "\t" + "node low"
			    + "\t" + "node high" + "\t" + "node CellLineName"
			    + "\t" + "node FileName"
			    + System.getProperty("line.separator"));
		}

		if (!dnaseCellLineNumber2OneorZeroMap.containsKey(castedNode
			.getCellLineNumber())) {
		    dnaseCellLineNumber2OneorZeroMap.put(
			    castedNode.getCellLineNumber(), (short) 1);
		}
		...
		bufferedWriter.write(chromName.convertEnumtoString()
			...
			+ ChromosomeName.convertEnumtoString(castedNode
				.getChromName()) + "\t" + castedNode.getLow()
				+ "\t" + castedNode.getHigh() + "\t" + cellLineName
				+ "\t" + fileName
				+ System.getProperty("line.separator"));
		bufferedWriter.flush();
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingDnaseIntervalsWithNumbers(outputFolder,
		    node.getLeft(), interval, chromName,
		    dnaseCellLineNumber2bufferedWriterHashMap,
		    dnaseCellLineNumber2OneorZeroMap, overlapDefinition,
		    cellLineNumber2CellLineNameMap, fileNumber2FileNameMap);
	}
    ...
    // For each search input line, each dnase cell line will have value 1 or 0
    // These 1 or 0's will be accumulated in dnaseCellLine2KMap
    public void findAllOverlappingDnaseIntervals(String outputFolder,
	    IntervalTreeNode node, Interval interval, ChromosomeName chromName,
	    Map<String, BufferedWriter> bufferedWriterHashMap,
	    List<String> dnaseCellLineNameList,
	    Map<String, Integer> dnaseCellLine2OneorZeroMap,
	    int overlapDefinition) {
	FileWriter fileWriter = null;
	BufferedWriter bufferedWriter = null;
	...
	if (overlaps(castedNode.getLow(), castedNode.getHigh(),
		interval.getLow(), interval.getHigh(), overlapDefinition)
		&& dnaseCellLineNameList.contains(castedNode.getCellLineName())) {
	    try {

		bufferedWriter = bufferedWriterHashMap.get(castedNode
			.getCellLineName());

		if (bufferedWriter == null) {
		    fileWriter = FileOperations.createFileWriter(outputFolder
			    + Commons.DNASE_ANNOTATION_DIRECTORY + "_"
			    + castedNode.getCellLineName() + ".txt");
		    bufferedWriter = new BufferedWriter(fileWriter);
		    bufferedWriterHashMap.put(castedNode.getCellLineName(),
			    bufferedWriter);
		    bufferedWriter.write("Searched for chr" + "\t"
			    + "given interval low" + "\t"
			    + "given interval high" + "\t"
		...
		if (dnaseCellLine2OneorZeroMap
			.get(castedNode.getCellLineName()) == null) {
		    dnaseCellLine2OneorZeroMap.put(
			    castedNode.getCellLineName(), 1);
		}
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingDnaseIntervals(outputFolder, node.getLeft(),
		    interval, chromName, bufferedWriterHashMap,
		    dnaseCellLineNameList, dnaseCellLine2OneorZeroMap,
		    overlapDefinition);
	}
	...
	if ((node.getRight().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getRight().getMax())
		&& (node.getLow() <= interval.getHigh())) {
	    findAllOverlappingDnaseIntervals(outputFolder, node.getRight(),
		    interval, chromName, bufferedWriterHashMap,
		    dnaseCellLineNameList, dnaseCellLine2OneorZeroMap,
		    overlapDefinition);
    ...
    public void findAllOverlappingUcscRefSeqGenesIntervalsWithIOWithNumbers(
	    ...
	    String type, GeneSetAnalysisType geneSetAnalysisType,
	...
	if (Commons.NCBI_GENE_ID.equals(type)) {
		...
		try {
			...
			if (castedNode.getIntervalName().isExon()) {
			    ...
			    listofGeneSetNumberContainingThisGeneId = geneId2ListofGeneSetNumberMap
				    .get(castedNode.getGeneEntrezId());

			    if (listofGeneSetNumberContainingThisGeneId != null) {
				for (int i = 0; i < listofGeneSetNumberContainingThisGeneId
					.size(); i++) {

				    geneSetNumber = listofGeneSetNumberContainingThisGeneId
					    .get(i);
					...
					bufferedWriter = permutationNumberKeggPathwayNumber2BufferedWriterMap
						.get(permutationNumberKeggPathwayNumber);
					...
					if (bufferedWriter == null) {
					    ...
					    fileWriter = FileOperations
						    .createFileWriter(
							    outputFolder
							    + Commons.ANNOTATE_PERMUTATIONS_FOR_EXON_BASED_KEGG_PATHWAY_ANALYSIS
							    + Commons.PERMUTATION
							    + permutationNumber
							    + System.getProperty("file.separator")
							    + Commons.PERMUTATION
					    ...
					    .put(permutationNumberKeggPathwayNumber,
						    bufferedWriter);

					}// End of IF: bufferedWriter is null

					if (!(permutationNumberKeggPathwayNumber2OneorZeroMap
						.containsKey(permutationNumberKeggPathwayNumber))) {
					...
					bufferedWriter = permutationNumberUserDefinedGeneSetNumber2BufferedWriterMap
						.get(permutationNumberUserDefinedGeneSetNumber);
					...
					if (bufferedWriter == null) {
					    ...
					    fileWriter = FileOperations
						    .createFileWriter(
							    outputFolder
							    + Commons.ANNOTATE_PERMUTATIONS_FOR_EXON_BASED_USERDEFINED_GENESET_ANALYSIS
							    + Commons.PERMUTATION
							    + permutationNumber
							    + System.getProperty("file.separator")
							    + Commons.PERMUTATION
					    ...
					    permutationNumberUserDefinedGeneSetNumber2BufferedWriterMap
					    .put(permutationNumberUserDefinedGeneSetNumber,
						    bufferedWriter);

					}// End of IF: bufferedWriter is null

					if (!(permutationNumberUserDefinedGeneSetNumber2OneorZeroMap
						.containsKey(permutationNumberUserDefinedGeneSetNumber))) {
			    ...
			    // listofGeneSetNumberContainingThisGeneId is not
			    // null
		    ...
		    else if (geneSetAnalysisType
			    .isRegulationBasedGeneSetAnalysis()) {
				...
				for (int i = 0; i < listofGeneSetNumberContainingThisGeneId
					...
					if (bufferedWriter == null) {
					    ...
					    fileWriter = FileOperations
						    .createFileWriter(
							    outputFolder
							    + Commons.ANNOTATE_PERMUTATIONS_FOR_REGULATION_BASED_KEGG_PATHWAY_ANALYSIS
							    + Commons.PERMUTATION
							    + permutationNumber
							    + System.getProperty("file.separator")
							    + Commons.PERMUTATION
					...
					if (bufferedWriter == null) {
					    ...
					    fileWriter = FileOperations
						    .createFileWriter(
							    outputFolder
							    + Commons.ANNOTATE_PERMUTATIONS_FOR_REGULATION_BASED_USERDEFINED_GENESET_ANALYSIS
							    + Commons.PERMUTATION
							    + permutationNumber
							    + System.getProperty("file.separator")
							    + Commons.PERMUTATION
			...
			if (listofGeneSetNumberContainingThisGeneId != null) {
				...
				if (geneSetType.isKeggPathway()) {
					...
					fileWriter = FileOperations
						.createFileWriter(
							outputFolder
							+ Commons.ANNOTATE_PERMUTATIONS_FOR_ALL_BASED_KEGG_PATHWAY_ANALYSIS
							+ Commons.PERMUTATION
							+ permutationNumber
							+ System.getProperty("file.separator")
							+ Commons.PERMUTATION
				...
				// UserDefinedGeneSet starts
					...
					fileWriter = FileOperations
						.createFileWriter(
							outputFolder
							+ Commons.ANNOTATE_PERMUTATIONS_FOR_ALL_BASED_USERDEFINED_GENESET_ANALYSIS
							+ Commons.PERMUTATION
							+ permutationNumber
							+ System.getProperty("file.separator")
							+ Commons.PERMUTATION
			    ...
			    // UserDefinedGeneSet does not matter) having this
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingUcscRefSeqGenesIntervalsWithIOWithNumbers(
		    outputFolder,
		    permutationNumber,
		    node.getLeft(),
		    interval,
    ...
    // These 1 or 0's will be accumulated in keggPathway2KMap
    // with IO
    public void findAllOverlappingUcscRefSeqGenesIntervals(String outputFolder,
	    int permutationNumber, IntervalTreeNode node, Interval interval,
	    ChromosomeName chromName,
	    Map<String, BufferedWriter> bufferedWriterHashMap,
	    Map<String, List<String>> geneId2KeggPathwayMap,
	    Map<String, Integer> permutationNumberKeggPathway2OneorZeroMap,
	    String type, KeggPathwayAnalysisType keggPathwayAnalysisType,
	    int overlapDefinition) {
	FileWriter fileWriter = null;
	BufferedWriter bufferedWriter = null;
	...
	if (Commons.NCBI_GENE_ID.equals(type)) {
		...
		try {
		    ...
		    if (keggPathwayAnalysisType
			    .isExonBasedKeggPathwayAnalysis()) {

			// exon based kegg pathway analysis
			if (castedNode.getIntervalName().isExon()) {

			    keggPathWayListContainingThisGeneId = geneId2KeggPathwayMap
				    .get(castedNode.getGeneEntrezId()
					    .toString());

			    if (keggPathWayListContainingThisGeneId != null) {
				for (int i = 0; i < keggPathWayListContainingThisGeneId
					.size(); i++) {
				    keggPathwayName = keggPathWayListContainingThisGeneId
					    .get(i);
				    permutationNumberKeggPathwayName = Commons.PERMUTATION
					    + permutationNumber
					    + "_"
					    + keggPathwayName;

				    bufferedWriter = bufferedWriterHashMap
					    .get(permutationNumberKeggPathwayName);
					...
					bufferedWriter = new BufferedWriter(
						fileWriter);
					bufferedWriterHashMap
					.put(permutationNumberKeggPathwayName,
						bufferedWriter);

				    }

				    if (permutationNumberKeggPathway2OneorZeroMap
					    .get(permutationNumberKeggPathwayName) == null) {
					permutationNumberKeggPathway2OneorZeroMap
					.put(permutationNumberKeggPathwayName,
						1);
				    }
		    ...
		    else if (keggPathwayAnalysisType
			    .isRegulationBasedKeggPathwayAnalysis()) {
			// Regulation Based kegg pathway analysis
			if (castedNode.getIntervalName().isIntron()
				|| castedNode.getIntervalName().isFivePOne()
				|| castedNode.getIntervalName().isFivePTwo()
			...
			keggPathWayListContainingThisGeneId = geneId2KeggPathwayMap
				.get(castedNode.getGeneEntrezId().toString());
			...
			if (keggPathWayListContainingThisGeneId != null) {
				...
				if (bufferedWriter == null) {
				    ...
				    bufferedWriter = new BufferedWriter(
					    fileWriter);
				    bufferedWriterHashMap.put(
					    permutationNumberKeggPathwayName,
					    bufferedWriter);
				...
				bufferedWriter.write("Searched for" + "\t"
					+ chromName + "\t" + interval.getLow()
					+ "\t" + interval.getHigh() + "\t"
					+ "ucscRefSeqGene" + "\t"
					+ castedNode.getChromName() + "\t"
					+ castedNode.getLow() + "\t"
					+ castedNode.getHigh() + "\t"
					+ castedNode.getRefSeqGeneName() + "\t"
					+ castedNode.getIntervalName() + "\t"
					+ castedNode.getGeneHugoSymbol() + "\t"
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingUcscRefSeqGenesIntervals(outputFolder,
		    permutationNumber, node.getLeft(), interval, chromName,
		    bufferedWriterHashMap, geneId2KeggPathwayMap,
		    permutationNumberKeggPathway2OneorZeroMap, type,
		    keggPathwayAnalysisType, overlapDefinition);
	}
	...
	if (Commons.NCBI_GENE_ID.equals(type)) {
		...
		if (geneSetAnalysisType.isExonBasedGeneSetAnalysis()) {
			...
			if (ListofGeneSetNumberContainingThisGeneId != null) {
				...
				if (geneSetType.isKeggPathway()) {
					...
					System.out
					.println("there is a situation 4");
				    }
				    // Debug end
				...
				// UserDefinedGeneSet starts
					...
					System.out
					.println("there is a situation 5");
				    }
				    // Debug end
		...
		// write REGULATION based results
			...
			if (ListofGeneSetNumberContainingThisGeneId != null) {
				...
				if (geneSetType.isKeggPathway()) {
				    ...
				    if (permutationNumberKeggPathwayNumber < 0) {
					System.out
					.println("there is a situation 6");
				    }
				    // Debug end
				...
				// UserDefinedGeneSet starts
				    ...
				    if (permutationNumberUserDefinedGeneSetNumber < 0) {
					System.out
					.println("there is a situation 7");
				    }
				    // Debug end
		...
		// write ALL based results starts
			...
			for (int i = 0; i < ListofGeneSetNumberContainingThisGeneId
				...
				if (permutationNumberKeggPathwayNumber < 0) {
				    System.out
				    .println("there is a situation 8");
				}
				// Debug end
				...
				if (permutationNumberUserDefinedGeneSetNumber < 0) {
				    System.out
				    .println("there is a situation 9");
				}
				// Debug end
		...
		// write all results
			...
			for (int i = 0; i < keggPathWayListContainingThisGeneId
			    ...
			    keggPathwayName = keggPathWayListContainingThisGeneId
				    .get(i);
			    permutationNumberKeggPathwayName = Commons.PERMUTATION
				    + permutationNumber + "_" + keggPathwayName;
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingUcscRefSeqGenesIntervals(permutationNumber,
		    node.getLeft(), interval, chromName, geneId2KeggPathwayMap,
		    permutationNumberKeggPathway2OneorZeroMap, type,
		    keggPathwayAnalysisType, overlapDefinition);
	}
	...
	if (Commons.NCBI_GENE_ID.equals(type)) {
		...
		try {
		    ...
		    keggPathwayNumberListContainingThisGeneId = geneId2KeggPathwayNumberMap
			    .get(castedNode.getGeneEntrezId());
			...
			if (keggPathwayNumberListContainingThisGeneId != null) {
			    ...
			    .add(new PermutationNumberUcscRefSeqGeneNumberOverlap(
				    ...
				    .getGeneHugoSymbolNumber(),
				    castedNode.getGeneEntrezId(),
				    castedNode.getLow(), castedNode
				    .getHigh(),
				    keggPathwayNumberListContainingThisGeneId));

			    for (int i = 0; i < keggPathwayNumberListContainingThisGeneId
				    .size(); i++) {

				keggPathwayNumber = keggPathwayNumberListContainingThisGeneId
					.get(i);
				...
				bufferedWriter = exonBasedKeggPathwayBufferedWriterHashMap
					.get(permutationNumberKeggPathwayNumber);
				...
				if (!(permutationNumberExonBasedKeggPathway2OneorZeroMap
					.containsKey(permutationNumberKeggPathwayNumber))) {
			...
			if (keggPathwayNumberListContainingThisGeneId != null) {
				...
				bufferedWriter = regulationBasedKeggPathwayBufferedWriterHashMap
					.get(permutationNumberKeggPathwayNumber);
				...
				if (!(permutationNumberRegulationBasedKeggPathway2OneorZeroMap
					.containsKey(permutationNumberKeggPathwayNumber))) {
			...
			for (int i = 0; i < keggPathwayNumberListContainingThisGeneId
				...
				bufferedWriter.write("Searched for" + "\t"
					+ "chromName" + "\t" + "intervalLow"
					+ "\t" + "intervalHigh" + "\t"
					+ "ucscRefSeqGene" + "\t" + "ChromName"
					+ "\t" + "Low" + "\t" + "High" + "\t"
					+ "RefSeqGeneNumber" + "\t"
					+ "IntervalName" + "\t"
					+ "IntervalNumber" + "\t"
				...
				allBasedKeggPathwayBufferedWriterHashMap.put(
					permutationNumberKeggPathwayNumber,
					bufferedWriter);
			    }

			    if (!(permutationNumberAllBasedKeggPathway2OneorZeroMap
				    .containsKey(permutationNumberKeggPathwayNumber))) {
				permutationNumberAllBasedKeggPathway2OneorZeroMap
				.put(permutationNumberKeggPathwayNumber,
					1);
			    }
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingUcscRefSeqGenesIntervalsWithIOWithNumbers(
		    outputFolder, permutationNumber, node.getLeft(), interval,
		    chromName, geneId2KeggPathwayNumberMap,
		    exonBasedKeggPathwayBufferedWriterHashMap,
		    regulationBasedKeggPathwayBufferedWriterHashMap,
	...
	if ((node.getRight().getNodeName().isNotSentinel())
		...
		&& (node.getLow() <= interval.getHigh())) {
	    findAllOverlappingUcscRefSeqGenesIntervalsWithIOWithNumbers(
		    outputFolder, permutationNumber, node.getRight(), interval,
		    chromName, geneId2KeggPathwayNumberMap,
		    exonBasedKeggPathwayBufferedWriterHashMap,
		    regulationBasedKeggPathwayBufferedWriterHashMap,
		    allBasedKeggPathwayBufferedWriterHashMap,
    ...
    public void findAllOverlappingUcscRefSeqGenesIntervals(
	    ...
	    Map<String, Integer> permutationNumberAllBasedKeggPathway2OneorZeroMap,
	    String type,
	...
	if (Commons.NCBI_GENE_ID.equals(type)) {
		...
		try {
			...
			if (keggPathwayListContainingThisGeneId != null) {
			    ...
			    permutationNumberExonBasedKeggPathwayOverlapList
			    .add(new PermutationNumberUcscRefSeqGeneOverlap(
				    Commons.PERMUTATION
				    + permutationNumber,
				    castedNode.getRefSeqGeneName(),
				    castedNode.getIntervalName(),
	...
	int repeatNumberReflectedPermutationNumber = ((repeatNumber - 1) * NUMBER_OF_PERMUTATIONS)
		+ permutationNumber;
				...
				if (bufferedWriter == null) {
				    ...
				    fileWriter = FileOperations
					    .createFileWriter(
						    ...
						    + Commons.ANNOTATE_PERMUTATIONS_FOR_EXON_BASED_KEGG_PATHWAY_ANALYSIS
						    + Commons.PERMUTATION
						    + repeatNumberReflectedPermutationNumber
						    + System.getProperty("file.separator")
						    + Commons.PERMUTATION
						    + repeatNumberReflectedPermutationNumber
						    + "_exonBased_"
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingUcscRefSeqGenesIntervals(outputFolder,
		    repeatNumber, permutationNumber, NUMBER_OF_PERMUTATIONS,
		    node.getLeft(), interval, chromName, geneId2KeggPathwayMap,
		    exonBasedKeggPathwayBufferedWriterHashMap,
		    regulationBasedKeggPathwayBufferedWriterHashMap,
		    allBasedKeggPathwayBufferedWriterHashMap,
	...
	if (Commons.NCBI_GENE_ID.equals(type)) {
		...
		keggPathwayListContainingThisGeneId = geneId2KeggPathwayNumbersMap
			.get(castedNode.getGeneEntrezId());
		...
		if (castedNode.getIntervalName().isExon()) {
			...
			for (int i = 0; i < keggPathwayListContainingThisGeneId
			    ...
			    keggPathwayNumber = keggPathwayListContainingThisGeneId
				    .get(i);
			    permutationNumberKeggPathwayNumber = generatePermutationNumberCellLineNumberorGeneSetNumber(
				    permutationNumber,
				    keggPathwayNumber,
			    ...
			    if (!(permutationNumberExonBasedKeggPathwayNumber2OneorZeroMap
				    .containsKey(permutationNumberKeggPathwayNumber))) {
		...
		if (castedNode.getIntervalName().isIntron()
			...
			for (int i = 0; i < keggPathwayListContainingThisGeneId
			    ...
			    if (!(permutationNumberRegulationBasedKeggPathwayNumber2OneorZeroMap
				    .containsKey(permutationNumberKeggPathwayNumber))) {
		...
		if (keggPathwayListContainingThisGeneId != null) {
		    ...
		    .add(new PermutationNumberUcscRefSeqGeneNumberOverlap(
			    permutationNumber, castedNode
			    .getRefSeqGeneNumber(), castedNode
			    .getIntervalName(), castedNode
			    .getIntervalNumber(), castedNode
			    .getGeneHugoSymbolNumber(),
			    castedNode.getGeneEntrezId(), castedNode
			    .getLow(), castedNode.getHigh(),
			    keggPathwayListContainingThisGeneId));
			...
			if (!(permutationNumberAllBasedKeggPathwayNumber2OneorZeroMap
				.containsKey(permutationNumberKeggPathwayNumber))) {
			    permutationNumberAllBasedKeggPathwayNumber2OneorZeroMap
			    .put(permutationNumberKeggPathwayNumber, 1);
			}
	...
	if (Commons.NCBI_GENE_ID.equals(type)) {
		...
		if (keggPathwayListContainingThisGeneId != null) {
		    ...
		    .add(new PermutationNumberUcscRefSeqGeneOverlap(
			    Commons.PERMUTATION + permutationNumber,
			    castedNode.getRefSeqGeneName(), castedNode
			    .getIntervalName(), castedNode
			    .getIntervalNumber(), castedNode
			    .getGeneHugoSymbol(), castedNode
			    .getGeneEntrezId(), castedNode
			    .getLow(), castedNode.getHigh(),
			    keggPathwayListContainingThisGeneId));
			...
			if (permutationNumberAllBasedKeggPathway2OneorZeroMap
				.get(permutationNumberKeggPathwayName) == null) {
			    permutationNumberAllBasedKeggPathway2OneorZeroMap
			    .put(permutationNumberKeggPathwayName, 1);
			}
    ...
    public void findAllGeneOverlappingUcscRefSeqGenesIntervalsWithNumbers(
	    String outputFolder, IntervalTreeNode node, Interval interval,
	    ChromosomeName chromName,
	    TIntShortMap geneAlternateNumber2OneorZeroMap, String type,
	    int overlapDefinition,
	...
	if (Commons.NCBI_GENE_ID.equals(type)) {
		...
		try {
		    ...
		    if (!geneAlternateNumber2OneorZeroMap
			    .containsKey(geneAlternateNumber)) {
			geneAlternateNumber2OneorZeroMap.put(
				geneAlternateNumber, (short) 1);
		    }
		    ...
		    bufferedWriter.write(chromName.convertEnumtoString()
			    ...
			    + ChromosomeName.convertEnumtoString(castedNode
				    ...
				    + castedNode.getHigh()
				    + "\t"
				    + refSeqGeneNumber2RefSeqGeneNameMap.get(castedNode
					    .getRefSeqGeneNumber())
					    + "\t"
					    + castedNode.getIntervalName()
					    .convertEnumtoString()
					    ...
					    .get(castedNode.getGeneHugoSymbolNumber())
					    + "\t" + castedNode.getGeneEntrezId()
					    + System.getProperty("line.separator"));
		    bufferedWriter.flush();
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
	    ...
	    findAllGeneOverlappingUcscRefSeqGenesIntervalsWithNumbers(
		    outputFolder, node.getLeft(), interval, chromName,
		    geneAlternateNumber2OneorZeroMap, type, overlapDefinition,
		    geneHugoSymbolNumber2GeneHugoSymbolNameMap,
    ...
    public void findAllOverlappingUcscRefSeqGenesIntervalsWithNumbers(
	    ...
	    TShortShortMap allBasedGeneSet2OneorZeroMap, String type,
	    int overlapDefinition,
	    TShortObjectMap<String> geneSetNumber2GeneSetNameMap,
	    TIntObjectMap<TShortList> geneId2ListofGeneSetNumberMap,
	    TIntObjectMap<String> geneHugoSymbolNumber2GeneHugoSymbolNameMap,
	    TIntObjectMap<String> refSeqGeneNumber2RefSeqGeneNameMap,
	    String geneSetName, GeneSetType geneSetType) {
	...
	if (Commons.NCBI_GENE_ID.equals(type)) {
		...
		geneSetNumberListContainingThisGeneId = geneId2ListofGeneSetNumberMap
			.get(castedNode.getGeneEntrezId());
		...
		try {
			...
			if (geneSetNumberListContainingThisGeneId != null) {
			    ...
			    for (TShortIterator it = geneSetNumberListContainingThisGeneId
				    .iterator(); it.hasNext();) {
				geneSetNumber = it.next();

				bufferedWriter = exonBasedGeneSetBufferedWriterHashMap
					.get(geneSetNumber);
				...
				if (bufferedWriter == null) {
				    ...
				    bufferedWriter = new BufferedWriter(
					    fileWriter);
				    exonBasedGeneSetBufferedWriterHashMap.put(
					    geneSetNumber, bufferedWriter);
				    bufferedWriter
				    .write("#Searched for chr"
					    + "\t"
					    + "interval Low"
					    + "\t"
					    + "interval High"
				...
				if (!exonBasedGeneSet2OneorZeroMap
					.containsKey(geneSetNumber)) {
				    exonBasedGeneSet2OneorZeroMap.put(
					    geneSetNumber, (short) 1);
				}
				...
				.write(chromName.convertEnumtoString()
					...
					+ "\t"
					+ ChromosomeName
					.convertEnumtoString(castedNode
						.getChromName())
						+ "\t"
						+ castedNode.getLow()
						+ "\t"
			...
			if (geneSetNumberListContainingThisGeneId != null) {
				...
				bufferedWriter = regulationBasedGeneSetBufferedWriterHashMap
					.get(geneSetNumber);
				...
				if (bufferedWriter == null) {
				    ...
				    bufferedWriter = new BufferedWriter(
					    fileWriter);
				    regulationBasedGeneSetBufferedWriterHashMap
				    .put(geneSetNumber, bufferedWriter);
				    bufferedWriter
				    .write("#Searched for chr"
					    + "\t"
					    + "interval Low"
				...
				if (!regulationBasedGeneSet2OneorZeroMap
					.containsKey(geneSetNumber)) {
				    regulationBasedGeneSet2OneorZeroMap.put(
					    geneSetNumber, (short) 1);
				}
			...
			for (TShortIterator it = geneSetNumberListContainingThisGeneId
			    ...
			    bufferedWriter = allBasedGeneSetBufferedWriterHashMap
				    .get(geneSetNumber);
				...
				bufferedWriter = new BufferedWriter(fileWriter);
				allBasedGeneSetBufferedWriterHashMap.put(
					geneSetNumber, bufferedWriter);
				bufferedWriter.write("#Searched for chr" + "\t"
					+ "interval Low" + "\t"
					+ "interval High" + "\t"
			    ...
			    if (!allBasedGeneSet2OneorZeroMap
				    .containsKey(geneSetNumber)) {
				allBasedGeneSet2OneorZeroMap.put(geneSetNumber,
					(short) 1);
			    }
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
	    ...
	    findAllOverlappingUcscRefSeqGenesIntervalsWithNumbers(outputFolder,
		    node.getLeft(), interval, chromName,
		    exonBasedGeneSetBufferedWriterHashMap,
		    regulationBasedGeneSetBufferedWriterHashMap,
		    allBasedGeneSetBufferedWriterHashMap,
		    exonBasedGeneSet2OneorZeroMap,
		    regulationBasedGeneSet2OneorZeroMap,
		    allBasedGeneSet2OneorZeroMap, type, overlapDefinition,
		    geneSetNumber2GeneSetNameMap,
		    geneId2ListofGeneSetNumberMap,
		    geneHugoSymbolNumber2GeneHugoSymbolNameMap,
		    refSeqGeneNumber2RefSeqGeneNameMap, geneSetName,
		    geneSetType);
	}
	...
	if (Commons.NCBI_GENE_ID.equals(type)) {
		...
		keggPathWayNumberListContainingThisGeneId = geneId2ListofKeggPathwayNumberMap
			.get(castedNode.getGeneEntrezId());
		...
		try {
			...
			if (keggPathWayNumberListContainingThisGeneId != null) {
			    ...
			    .add(new UcscRefSeqGeneOverlapWithNumbers(
				    castedNode.getRefSeqGeneNumber(),
				    castedNode
				    .getGeneHugoSymbolNumber(),
				    castedNode.getGeneEntrezId(),
				    ...
				    castedNode.getIntervalName(),
				    castedNode.getIntervalNumber(),
				    node.getLow(), node.getHigh()));

			    for (TShortIterator it = keggPathWayNumberListContainingThisGeneId
				    .iterator(); it.hasNext();) {
				keggPathwayNumber = it.next();

				bufferedWriter = exonBasedKeggPathwayBufferedWriterHashMap
					.get(keggPathwayNumber);

				if (bufferedWriter == null) {
				    fileWriter = FileOperations
					    .createFileWriter(
						    outputFolder
						    + Commons.EXON_BASED_KEGG_PATHWAY_ANNOTATION
						    + "_"
				...
				if (!exonBasedKeggPathway2OneorZeroMap
					.containsKey(keggPathwayNumber)) {
				    exonBasedKeggPathway2OneorZeroMap.put(
					    keggPathwayNumber, (short) 1);
				}
			...
			if (keggPathWayNumberListContainingThisGeneId != null) {
				...
				if (bufferedWriter == null) {
				    fileWriter = FileOperations
					    .createFileWriter(
						    outputFolder
						    + Commons.REGULATION_BASED_KEGG_PATHWAY_ANNOTATION
						    + "_"
						    + keggPathwayNumber2KeggPathwayNameMap
						    .get(keggPathwayNumber)
						    + ".txt", true);
				...
				if (!regulationBasedKeggPathway2OneorZeroMap
					.containsKey(keggPathwayNumber)) {
				    regulationBasedKeggPathway2OneorZeroMap
				    .put(keggPathwayNumber, (short) 1);
				}
			...
			for (TShortIterator it = keggPathWayNumberListContainingThisGeneId
				...
				bufferedWriter = new BufferedWriter(fileWriter);
				allBasedKeggPathwayBufferedWriterHashMap.put(
					keggPathwayNumber, bufferedWriter);
				bufferedWriter.write("#Searched for chr" + "\t"
					+ "interval Low" + "\t"
					+ "interval High" + "\t"
					+ "ucscRefSeqGene node ChromName"
			    ...
			    if (!allBasedKeggPathway2OneorZeroMap
				    .containsKey(keggPathwayNumber)) {
				allBasedKeggPathway2OneorZeroMap.put(
					keggPathwayNumber, (short) 1);
			    }
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
	    ...
	    findAllOverlappingUcscRefSeqGenesIntervalsWithNumbers(outputFolder,
		    ...
		    regulationBasedKeggPathway2OneorZeroMap,
		    allBasedKeggPathway2OneorZeroMap, type,
		    exonBasedKeggPathwayOverlapList,
		    regulationBasedKeggPathwayOverlapList,
    ...
    public void findAllOverlappingUcscRefSeqGenesIntervals(
	    ...
	    List<UcscRefSeqGeneOverlap> allBasedKeggPathwayOverlapList,
	    int overlapDefinition) {
	FileWriter fileWriter = null;
	BufferedWriter bufferedWriter = null;
	...
	if (Commons.NCBI_GENE_ID.equals(type)) {
		...
		try {
			...
			if (keggPathWayListContainingThisGeneId != null) {
			    ...
			    exonBasedKeggPathwayOverlapList
			    .add(new UcscRefSeqGeneOverlap(castedNode
				    .getRefSeqGeneName(), castedNode
				    .getIntervalName(), castedNode
				    .getIntervalNumber(), castedNode
				    .getGeneHugoSymbol(), castedNode
				...
				if (keggPathwayNameList
					.contains(keggPathwayName)) {
			...
			for (int i = 0; i < keggPathWayListContainingThisGeneId
				...
				bufferedWriter.write(chromName
					.convertEnumtoString()
					+ "\t"
					+ interval.getLow()
					+ "\t"
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
	    ...
	    findAllOverlappingUcscRefSeqGenesIntervals(outputFolder,
		    ...
		    regulationBasedKeggPathwayBufferedWriterHashMap,
		    allBasedKeggPathwayBufferedWriterHashMap,
		    geneId2KeggPathwayMap, keggPathwayNameList,
		    exonBasedKeggPathway2OneorZeroMap,
		    regulationBasedKeggPathway2OneorZeroMap,
		    allBasedKeggPathway2OneorZeroMap, type,
		    exonBasedKeggPathwayOverlapList,
    ...
    public void findAllOverlappingUcscRefSeqGenesIntervals(
	    ...
	    Map<String, List<String>> geneId2KeggPathwayMap,
	    List<String> keggPathwayNameList,
	    Map<String, Integer> keggPathway2OneorZeroMap, String type,
		...
		try {
			...
			if (castedNode.getIntervalName().isExon()) {
				...
				for (int i = 0; i < keggPathWayListContainingThisGeneId
					...
					if (bufferedWriter == null) {
					    fileWriter = FileOperations
						    .createFileWriter(Commons.EXON_BASED_KEGG_PATHWAY_ANNOTATION
							    + "_exonBased_"
							    + keggPathwayName
							    + ".txt");
					    bufferedWriter = new BufferedWriter(
					...
					if (keggPathway2OneorZeroMap
						.get(keggPathwayName) == null) {
					    keggPathway2OneorZeroMap.put(
						    keggPathwayName, 1);
					}
			...
			if (castedNode.getIntervalName().isIntron()
				...
				for (int i = 0; i < keggPathWayListContainingThisGeneId
					...
					if (bufferedWriter == null) {
					    fileWriter = FileOperations
						    .createFileWriter(Commons.REGULATION_BASED_KEGG_PATHWAY_ANNOTATION
							    + "_regulationBased_"
							    + keggPathwayName
							    + ".txt");
					    bufferedWriter = new BufferedWriter(
			...
			if (keggPathWayListContainingThisGeneId != null) {
				...
				if (keggPathwayNameList
				    ...
				    if (bufferedWriter == null) {
					fileWriter = FileOperations
						.createFileWriter(Commons.ALL_BASED_KEGG_PATHWAY_ANALYSIS
							+ "_all_"
							+ keggPathwayName
							+ ".txt");
					bufferedWriter = new BufferedWriter(
	...
	if ((node.getLeft().getNodeName().isNotSentinel())
		&& (interval.getLow() <= node.getLeft().getMax())) {
	    findAllOverlappingUcscRefSeqGenesIntervals(
		    (UcscRefSeqGeneIntervalTreeNode) node.getLeft(), interval,
		    chromName, bufferedWriterHashMap, geneId2KeggPathwayMap,
		    keggPathwayNameList, keggPathway2OneorZeroMap, type,
		    keggPathwayAnalysisType);
	}
	...
	if ((node.getRight().getNodeName().isNotSentinel())
		...
		&& (node.getLow() <= interval.getHigh())) {
	    findAllOverlappingUcscRefSeqGenesIntervals(
		    (UcscRefSeqGeneIntervalTreeNode) node.getRight(), interval,
		    chromName, bufferedWriterHashMap, geneId2KeggPathwayMap,
		    keggPathwayNameList, keggPathway2OneorZeroMap, type,
		    keggPathwayAnalysisType);
	...
	try {
		...
		if (Commons.NCBI_GENE_ID.equals(type)) {
		    if (overlaps(castedNode.getLow(), castedNode.getHigh(),
			    interval.getLow(), interval.getHigh())
			    && nameorIdHashMap.containsKey(castedNode
				    .getGeneEntrezId().toString())) {

			bufferedWriter = bufferedWriterHashMap.get(castedNode
				.getGeneEntrezId().toString());
			count = nameorIdHashMap.get(castedNode
				.getGeneEntrezId().toString());

			if (bufferedWriter == null) {
			    fileWriter = FileOperations
				    .createFileWriter(Commons.NCBI_GENE_ID_ANNOTATION_DIRECTORY
					    + "_"
					    + String.valueOf(castedNode
						    .getGeneEntrezId())
						    + ".txt");
			    bufferedWriter = new BufferedWriter(fileWriter);
			    bufferedWriterHashMap.put(castedNode
				    .getGeneEntrezId().toString(),
				    bufferedWriter);
			...
			count++;
			nameorIdHashMap.put(castedNode.getGeneEntrezId()
				.toString(), count);
		    }
		} else if (Commons.NCBI_RNA_NUCLEOTIDE_ACCESSION_VERSION
			.equals(type)) {
		    if (overlaps(castedNode.getLow(), castedNode.getHigh(),
			    interval.getLow(), interval.getHigh())
			    && nameorIdHashMap.containsKey(castedNode
			...
			if (bufferedWriter == null) {
			    fileWriter = FileOperations
				    .createFileWriter(Commons.NCBI_RNA_ANNOTATION_DIRECTORY
					    + "_"
					    + castedNode.getRefSeqGeneName()
					    + ".txt");
			    bufferedWriter = new BufferedWriter(fileWriter);
			...
			count++;
			nameorIdHashMap.put(castedNode.getRefSeqGeneName(),
				count);
			...
			if (bufferedWriter == null) {
			    fileWriter = FileOperations
				    .createFileWriter(Commons.UCSC_GENE_ALTERNATE_NAME_ANNOTATION_DIRECTORY
					    + "_"
					    + castedNode.getGeneHugoSymbol()
					    + ".txt");
			    bufferedWriter = new BufferedWriter(fileWriter);
			...
			count++;
			nameorIdHashMap.put(castedNode.getGeneHugoSymbol(),
				count);
		    }
		}
		...
		if ((node.getLeft().getNodeName().isNotSentinel())
			&& (interval.getLow() <= node.getLeft().getMax())) {
		    findAllOverlappingUcscRefSeqGenesIntervals(
			    (UcscRefSeqGeneIntervalTreeNode) node.getLeft(),
			    interval, chromName, bufferedWriterHashMap,
			    nameorIdHashMap, type);
		}
		...
		if ((node.getRight().getNodeName().isNotSentinel())
			...
			&& (node.getLow() <= interval.getHigh())) {
		    findAllOverlappingUcscRefSeqGenesIntervals(
			    (UcscRefSeqGeneIntervalTreeNode) node.getRight(),
			    interval, chromName, bufferedWriterHashMap,
			    nameorIdHashMap, type);
    ...
    // Search1
    public void findAllOverlappingUcscRefSeqGenesIntervals(
	    IntervalTreeNode node, Interval interval,
	    BufferedWriter bufferedWriter,
	    List<IntervalTreeNode> overlappingNodeList) {
	...
	if (overlaps(node.getLow(), node.getHigh(), interval.getLow(),
	    ...
	    try {
		bufferedWriter.write("ucscRefSeqGene" + "\t"
			+ castedNode.getChromName().toString() + "\t"
			+ castedNode.getLow() + "\t" + castedNode.getHigh()
			+ "\t" + castedNode.getRefSeqGeneName().toString()
			+ "\t" + castedNode.getIntervalName().toString() + "\t"
			+ castedNode.getGeneHugoSymbol().toString() + "\t"
			+ castedNode.getGeneEntrezId()
			+ System.getProperty("line.separator"));
		bufferedWriter.flush();
	...
	for (int i = 0; i < path.getNodeList().size(); i++) {
	    ...
	    GlanetRunner.appendLog("Node(" + node.getLow() + ","
		    + node.getHigh() + ",Max: " + node.getMax() + ") Color: "
		    + node.getColor() + " Name: "
		    + node.getNodeName().convertEnumtoString());

	}

	GlanetRunner.appendLog("--- Number of black nodes: "
		+ numberofBlackNodes);

    }

    public static void findAllPathsFromRootToLeavesUsingDepthFirstSearch(
	    IntervalTreeNode node, IntervalTreeProperties properties,
	    Path path, List<Path> allPathsFromRootToLeaves) {
	...
	// SENTINEL or NOT_SENTINEL_NODE
		...
		Path onePathFromRootToLeaf = new Path(
			path.getNumberofBlackNodes(), path.getNodeList());
		...
		if (node.getColor() != Commons.RED
			&& node.getColor() != Commons.BLACK) {
		    properties.setEveryNodeIsEitherRedorBlack(false);
		    return;
		}

		if (node.getColor() == Commons.RED) {
		    if (node.getLeft().getColor() != Commons.BLACK
			    && node.getRight().getColor() != Commons.BLACK) {
		...
		findAllPathsFromRootToLeavesUsingDepthFirstSearch(
			node.getLeft(), properties, path,
			allPathsFromRootToLeaves);
		findAllPathsFromRootToLeavesUsingDepthFirstSearch(
			node.getRight(), properties, path,
			allPathsFromRootToLeaves);
    ...
    public static void checkNumberofBlackNodesinAllPathsFromRootToLeaves(
	    IntervalTreeProperties properties, List<Path> allPaths) {
	int numberofBlackNodes = 0;
	...
	for (int i = 1; i < allPaths.size(); i++) {
		...
		properties
		.setAllPathsfromRoottoLeavesHasSameNumberofBlackNodes(false);
		break;
	    }
	}
    }
    ...
    public static void checkProperties(IntervalTree tree,
	    IntervalTreeProperties properties) {
	Path path = new Path();
	List<Path> allPathsFromRoottoLeaves = new ArrayList<Path>();

	findAllPathsFromRootToLeavesUsingDepthFirstSearch(tree.getRoot(),
		properties, path, allPathsFromRoottoLeaves);

	checkNumberofBlackNodesinAllPathsFromRootToLeaves(properties,
		allPathsFromRoottoLeaves);
	...
	intevalTreePropertiesHold = properties.isEveryNodeIsEitherRedorBlack()
		...
		&& properties
		.isAllPathsfromRoottoLeavesHasSameNumberofBlackNodes()
    ...
    public static void intervalTreeInsertDelete(IntervalTree tree,
	    IntervalTreeNode node, String operation) {

	boolean isIntervalTreePropertiesHold;
	IntervalTreeNode splicedOutNode;

	if (Commons.INSERT.equals(operation)) {
	    GlanetRunner.appendLog("After insert node (" + node.getLow() + ","
		    + node.getHigh() + ")");
	    tree.intervalTreeInsert(tree, node);
	    GlanetRunner.appendLog("Tree Root color: "
		    + tree.getRoot().getColor()
		    + " Tree Root Low: "
		    + tree.getRoot().getLow()
		    + " Tree Root High: "
		    + tree.getRoot().getHigh()
		    + " Tree Root Max: "
		    + tree.getRoot().getMax()
		    + " Tree Root's Parent's Name: "
	    ...
	    GlanetRunner.appendLog("Does the interval tree properties hold? "
		    + isIntervalTreePropertiesHold);

	} else if (Commons.DELETE.equals(operation)) {
	    GlanetRunner.appendLog("After delete node (" + node.getLow() + ","
		    + node.getHigh() + ")");
	    splicedOutNode = tree.intervalTreeDelete(tree, node);
	    GlanetRunner.appendLog("splicedOutNode color: "
		    + splicedOutNode.getColor() + " splicedOutNode Low: "
		    + splicedOutNode.getLow() + " splicedOutNode High: "
		    + splicedOutNode.getHigh() + " splicedOutNode Max: "
		    + splicedOutNode.getMax());
	    splicedOutNode = null;
	    GlanetRunner.appendLog("Tree Root color: "
		    + tree.getRoot().getColor()
		    + " Tree Root Low: "
	...
	System.out.println("isIntervalTreePropertiesHold: "
		+ isIntervalTreePropertiesHold);
