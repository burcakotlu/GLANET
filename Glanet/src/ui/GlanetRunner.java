...
//		/************************ Annotation Binary Matrices ends *********************************/
		...
		if( getArgs()[CommandLineArguments.PerformEnrichment.value()].equalsIgnoreCase( Commons.DO_ENRICH)){
			...
			if( getArgs()[CommandLineArguments.WriteAnnotationFoundOverlapsMode.value()].equalsIgnoreCase(Commons.DO_WRITE_OVERLAPS_EACH_ONE_IN_SEPARATE_FILE_ELEMENT_BASED)){
				setCurrentProcessInfo("Write Element Based Annotation Found Overlaps in GRCh37.p13 ...");
			}
			
			if( getArgs()[CommandLineArguments.WriteAnnotationFoundOverlapsMode.value()].equalsIgnoreCase(Commons.DO_WRITE_OVERLAPS_ALL_IN_ONE_FILE_ELEMENT_TYPE_BASED)){
...
//				AugmentationofEnrichmentWithAnnotationInGRCh37p13Coordinates.main( args);
			...
			if( getArgs()[CommandLineArguments.WriteAnnotationFoundOverlapsMode.value()].equalsIgnoreCase(Commons.DO_NOT_WRITE_OVERLAPS_AT_ALL)){
