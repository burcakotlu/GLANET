...
public class FivePrimeThreePrime {
	...
	// 5p1, 5p2, 5d is always from gene start to five prime which is upstream of
	// a refseq gene
	// 3p1, 3p2, 3d is always from gene end to three prime which is downstream
	// of a refseq gene

	// For + strand, gene start at transcription start position
	// For + strand ----5prime---txStart---------txEnd-----3prime-------------
	// gene flows from 5 to 3 prime therefore gene starts at txStart

	// For - strand, gene start at transcription end position
	// For - strand ----3prime---txStart---------txEnd-----5prime-------------
	// gene flows from 5 to 3 prime therefore gene starts at txEnd

	// For + strand, from transcription start position to 5 prime is the
	// upstream of a refseq gene
	// For + strand, from transcription end position to 3 prime is the
	// downstream of a refseq gene

	// For - strand, from transcription end position to 5 prime is the upsteam
	// of a refseq gene
	// For - strand, from transcription start position to 3 prime is the
	// downstream of a refseq gene

	// Since gene always flow from 5 prime to 3 prime
	// important point is where the gene starts whether at transcription start
	// position or transcription end position
